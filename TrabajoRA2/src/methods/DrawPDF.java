...
public class DrawPDF {
	...
	try {
	    ...
	    for (Transaction t : Test.transactionList) {
	    	float totalPrice = Test.product.getProduct(Conexion.obtain(), t.getId_prod()).getPrice() * t.getAmount() ;
	    	finalPrice += totalPrice;
	    	Row<PDPage> row = table.createRow(12);
	    	cell = row.createCell(25, Test.product.getProduct(Conexion.obtain(), t.getId_prod()).getName() );
	    	cell = row.createCell(25, String.valueOf( Test.product.getProduct(Conexion.obtain(), t.getId_prod()).getPrice()));
